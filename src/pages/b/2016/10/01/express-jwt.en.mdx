...
Stateless authentication, on the other hand, relies on the user providing _all_ the information about their own identity. **The server doesn't need to keep track of any data about users that have "logged in"** as in the previous case. Everything the server know about the current user is sent by the users themselves. Obviously, this implies that the server has to somehow trust that the information the user is sending is reliable.
